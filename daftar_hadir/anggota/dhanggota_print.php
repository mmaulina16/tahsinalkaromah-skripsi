...
<head>
    <title>Cetak Daftar Hadir Anggota</title>
...
<body>
    ...
    $filter_npm = isset($_GET['filter_npm']) ? $_GET['filter_npm'] : '';
    $filter_bln = isset($_GET['filter_bln']) ? $_GET['filter_bln'] : '';

    // Query SQL dengan JOIN
    $query = "SELECT dh.id_dhanggota, a.npm, a.nama_anggota, dh.tanggal, dh.status_hadir 
          FROM daftarhadir_anggota dh
          JOIN anggota a ON dh.npm = a.npm
          WHERE 1=1";

    if ($filter_npm != '') {
        $query .= " AND a.npm = '" . mysqli_real_escape_string($koneksi, $filter_npm) . "'";
    }
    if ($filter_bln != '') {
        $query .= " AND MONTH(dh.tanggal) = '" . mysqli_real_escape_string($koneksi, $filter_bln) . "'";
    }
    ?>
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                ...
                <hr>
                <h3 style="text-align:center;">LAPORAN DAFTAR HADIR ANGGOTA</h3>
                <hr>
